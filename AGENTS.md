...
- If handling common operations that other projects will need, for example config, event sending, pairing flows, event receiving, gateway protocols, etc, always use `crab-sdk` or extend `crab-sdk`. `crab-sdk` is a central library for common operations and types.
...
- Config loading and saving is the responsibility of `crab-sdk`
...
- File contents should match their name. A generic helpers used by other files should not be within a domain-specific file. For example, "EnvOrDefault" living in "gateway.go" is a bad pattern. Use a dedicated "common.go" or "helpers.go" file.