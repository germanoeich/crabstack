...
func TestClaudeCompleteSuccess(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("x-api-key"); got != "" {
			t.Fatalf("expected x-api-key to be unset, got %s", got)
		}
		...
		if got := r.Header.Get("anthropic-beta"); got != claudeOAuthBeta {
			t.Fatalf("expected anthropic-beta=%s, got %s", claudeOAuthBeta, got)
		}
		if got := r.Header.Get("x-app"); got != "cli" {
			t.Fatalf("expected x-app=cli, got %s", got)
		}
	...
	provider := NewClaudeProvider(
		"test-token",
		WithClaudeEndpoint(server.URL+"/v1/messages"),
		WithClaudeHTTPClient(server.Client()),
	)
...
func TestClaudeCompleteRequestIncludesToolsAndToolResultMessages(t *testing.T) {
	var seen anthropicRequest
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if err := json.NewDecoder(r.Body).Decode(&seen); err != nil {
			t.Fatalf("decode request body: %v", err)
...
func TestClaudeCompleteRateLimit(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestClaudeCompleteMalformedJSON(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "decode claude response") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestClaudeCompleteInvalidRoleFailsBeforeRequest(t *testing.T) {
...
func TestClaudeCompleteRequiresAuthToken(t *testing.T) {
	provider := NewClaudeProvider("")
	...
	if err == nil {
		t.Fatalf("expected missing auth token error")
	}
	if err.Error() != "claude auth token is required" {
		t.Fatalf("unexpected error: %v", err)
	}
}
