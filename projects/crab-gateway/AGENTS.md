...
- `CRAB_GATEWAY_KEY_DIR` (default `.crabstack/keys` when local `.crabstack` exists, otherwise `~/.crabstack/keys`)
- `CRAB_GATEWAY_ADMIN_SOCKET_PATH` (default `.crabstack/run/gateway-admin.sock` when local `.crabstack` exists, otherwise `~/.crabstack/run/gateway-admin.sock`)
...
- `CRAB_GATEWAY_AGENTS_JSON` (optional JSON array override for gateway agent definitions)
