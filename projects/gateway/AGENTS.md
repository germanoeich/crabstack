...
## Scope
This project implements the Pinchy gateway runtime:
...
## Module
- Go module: `pinchy.local/projects/gateway`
...
## Config keys
- `PINCHY_GATEWAY_HTTP_ADDR` (default `:8080`)
- `PINCHY_GATEWAY_DB_DRIVER` (`sqlite` default, `postgres` supported)
- `PINCHY_GATEWAY_DB_DSN` (default `gateway.db` for sqlite)
- `PINCHY_GATEWAY_ID` (default `gateway-core`)
- `PINCHY_GATEWAY_KEY_DIR` (default `.pinchy/keys`)
- `PINCHY_GATEWAY_ADMIN_SOCKET_PATH` (default `.pinchy/run/gateway-admin.sock`)
- `PINCHY_GATEWAY_PAIR_TIMEOUT` (default `15s`)
- `PINCHY_GATEWAY_REQUIRE_MTLS_REMOTE` (default `true`)
- `PINCHY_GATEWAY_ALLOW_INSECURE_LOOPBACK_PAIRING` (default `true`, loopback-only dev mode)
- `PINCHY_GATEWAY_PAIR_MTLS_CA_FILE` (optional; must be set with cert/key when used)
- `PINCHY_GATEWAY_PAIR_MTLS_CERT_FILE` (optional; must be set with ca/key when used)
- `PINCHY_GATEWAY_PAIR_MTLS_KEY_FILE` (optional; must be set with ca/cert when used)

## Package map
- `cmd/gateway`: process bootstrap and wiring.
