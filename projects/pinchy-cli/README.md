...
Terminal UI client for Pinchy pairing + event exchange.
...
From `projects/pinchy-cli`:

```bash
PINCHY_CLI_GATEWAY_PUBLIC_KEY_ED25519='<gateway-ed25519-pub-b64>' \
GOWORK=off go run ./cmd/pinchy-cli pair \
  -admin-socket .pinchy/run/gateway-admin.sock \
  -component-type tool_host \
  -component-id pinchy-cli
```
...
```bash
PINCHY_CLI_GATEWAY_PUBLIC_KEY_ED25519='<gateway-ed25519-pub-b64>' \
GOWORK=off go run ./cmd/pinchy-cli \
