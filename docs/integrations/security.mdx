...
- Paired CLI principals (`component_type=operator`) can hold `cli_admin` policy, but cannot initiate pairing remotely.
- Authorization should evaluate as deny-by-default with explicit deny precedence.
...
See [Peer Auth and Authorization](./peer-auth.mdx) for role defaults, policy evaluation order, and temporary lease scope.
