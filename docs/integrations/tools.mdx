...
For `crab-cron`-specific behavior, see [Cron Service](./cron.mdx).
...
- Runtime calls to `/v1/tools/*` should be denied until pairing succeeds.
- Current gateway URL-discovery path (`CRAB_GATEWAY_TOOL_HOST_URLS`) is direct HTTP(S) routing and does not enforce pairing in that path yet.
