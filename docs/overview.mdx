...
- `channel listeners`: platform adapters for Discord/WhatsApp/etc that publish `channel.message.*`.
- `discord service` (`crab-discord`): Discord inbound listener + outbound webhook consumer.
- `memory service`: subscriber ingest plus optional tool host.
- `cron service` (`crab-cron`): emits scheduled events and exposes `cron.*` tools.
...
4. Pair and attach tool hosts (memory, cron, custom tools).
5. Add Discord listener/consumer wiring and validate end-to-end channel routing.
6. Expand to additional listeners and WS ingress.
